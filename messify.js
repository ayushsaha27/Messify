...
(function () {
  ...
  if (submitBtn) {
    ...
    fetch("/api/feedback/status?email=" + encodeURIComponent(userEmail))
      ...
      .then(function (data) {
        if (!data.submitted) {
          ...
          lockFeedbackForm();
          showToast(
            "You have already submitted this week. You can view your ratings but cannot change them.",
            "error",
          );
        }
      })
    ...
    window.submitFeedback = function () {
      if (submitBtn.disabled) {
        showToast(
          "You have already submitted this week. Ratings cannot be changed.",
          "error",
        );
        return;
      }
  ...
  if (document.getElementById("mealScores") && userRole === "admin") {
    ...
    fetch("/api/analytics/current")
      ...
      .then(function (res) {
        ...
        if (kpiVals.length >= 4) {
          ...
          if (kpiLabels[3])
            kpiLabels[3].textContent = "Worst — " + mealNames[d.worstMeal];
        }
          ...
          if (complaintCard) {
            ...
            res.data.slice(0, 5).forEach(function (c) {
              ...
              complaintCard.innerHTML +=
                ...
                '<div class="ci-ic" style="background:rgba(239,68,68,0.1)">' +
                icon +
                "</div>" +
    ...
    allBtns.forEach(function (btn) {
      if (
        btn.textContent.indexOf("Export") > -1 ||
        btn.textContent.indexOf("PDF") > -1
      ) {
        exportBtn = btn;
      }
    });
      ...
      exportBtn.onclick = function (e) {
        ...
        fetch("/api/admin/export-pdf", { headers: adminHeaders() })
          ...
          .catch(function (err) {
            ...
            exportBtn.disabled = false;
            showToast(
              "PDF error: " + err.message + ". Make sure server is running.",
              "error",
            );
          });
      };
