...
function getCurrentWeekInfo() {
  ...
  return { key, label, range, weekNum: wnum, year: now.getFullYear(), acadYear };
}
...
async function computeAnalytics(weekKey) {
  ...
  const mealAvg = {};
  MEALS.forEach(m => { mealAvg[m] = mealCount[m] > 0 ? Math.round((mealSum[m]/mealCount[m])*10)/10 : 0; });

  const valid = MEALS.filter(m => mealCount[m] > 0);
  const overallAvg = valid.length > 0 ? Math.round((valid.reduce((a,m)=>a+mealAvg[m],0)/valid.length)*10)/10 : 0;
  const sorted = [...MEALS].sort((a,b) => mealAvg[b]-mealAvg[a]);

  const comments = feedbacks.filter(f=>f.liked||f.issues)
    .map(f => ({liked:f.liked||'', issue:f.issues||'', date:f.submitted_at}));

  return {
    weekKey, weekLabel:feedbacks[0].week_label, weekRange:feedbacks[0].week_range,
    total:feedbacks.length, overallAvg, mealAvg,
    bestMeal:sorted[0], worstMeal:sorted[sorted.length-1],
    heatmap, comments
  };
}
  ...
  try {
    ...
    try {
      ...
      if(fs.existsSync(logoPath)){
        NIST_LOGO='data:image/png;base64,'+fs.readFileSync(logoPath).toString('base64');
      }
    } catch(e){}
    ...
    const likedC=an.comments.filter(c=>c.liked).map(c=>`<li style="padding:7px 0;border-bottom:1px solid rgba(255,255,255,.06);color:#d1d5db;font-size:13px;line-height:1.6">💬 ${escHtml(c.liked)}</li>`).join('');
    const issueC=an.comments.filter(c=>c.issue).map(c=>`<li style="padding:7px 0;border-bottom:1px solid rgba(255,255,255,.06);color:#d1d5db;font-size:13px;line-height:1.6">⚠️ ${escHtml(c.issue)}</li>`).join('');
    const wmStyle=NIST_LOGO?`background:url('${NIST_LOGO}') center/contain no-repeat;`:'background:radial-gradient(circle,rgba(249,115,22,.08) 0%,transparent 70%);';
...
<h2>⭐ Meal Averages</h2>
<div class="meal-row">${MEALS.map(m=>{const v=an.mealAvg[m];const c=v>=4?'var(--success)':v>=3?'var(--warning)':'var(--danger)';return `<div class="meal-card"><div class="meal-icon">${mealIcon[m]}</div><div class="meal-nm">${mealName[m]}</div><div class="meal-sc" style="color:${c}">${v}</div></div>`;}).join('')}</div>
...
<div class="comm-wrap"><div class="comm-grid">
  ...
  <div class="comm-sec"><h3 style="color:var(--danger)">⚠️ Issues raised (${an.comments.filter(c=>c.issue).length})</h3><ul>${issueC||'<li style="color:var(--muted);padding:8px 0">No issues raised this week.</li>'}</ul></div>
    ...
    res.send(html);
  } catch(e){ console.error('PDF error:',e); res.status(500).send('Error: '+e.message); }
});
