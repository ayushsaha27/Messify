// ═══════════════════════════════════════════════════════════
//  Messify Backend — server.js (Full 800+ Line Restored Version)
...
async function computeAnalytics(weekKey) {
  ...
  const heatVegSum = {}; const heatVegCount = {};
  const heatNvSum = {}; const heatNvCount = {};

  DAYS.forEach((d) => {
    heatSum[d] = { b: 0, l: 0, s: 0, d: 0 }; heatCount[d] = { b: 0, l: 0, s: 0, d: 0 };
    heatVegSum[d] = { b: 0, l: 0, s: 0, d: 0 }; heatVegCount[d] = { b: 0, l: 0, s: 0, d: 0 };
    heatNvSum[d] = { b: 0, l: 0, s: 0, d: 0 }; heatNvCount[d] = { b: 0, l: 0, s: 0, d: 0 };
  });
  ...
  feedbacks.forEach((fb) => {
    (fb.meal_ratings || []).forEach((r) => {
      const day = r.day; const meal = r.meal; const v = Number(r.rating || 0); const ft = r.food_type || "veg";
      if (v > 0 && heatSum[day] && MEAL_KEYS[meal]) {
        ...
        if (ft === "veg") { heatVegSum[day][k] += v; heatVegCount[day][k] += 1; } 
        else { heatNvSum[day][k] += v; heatNvCount[day][k] += 1; }
      }
    });
  });

  // Calculate averages
  const heatmap = {}; const heatVeg = {}; const heatNv = {};
  DAYS.forEach((d) => {
    heatmap[d] = {}; heatVeg[d] = {}; heatNv[d] = {};
    MEALS.forEach((m) => {
      const k = MEAL_KEYS[m];
      heatmap[d][k] = heatCount[d][k] > 0 ? Math.round((heatSum[d][k] / heatCount[d][k]) * 10) / 10 : 0;
      heatVeg[d][k] = heatVegCount[d][k] > 0 ? Math.round((heatVegSum[d][k] / heatVegCount[d][k]) * 10) / 10 : 0;
      heatNv[d][k] = heatNvCount[d][k] > 0 ? Math.round((heatNvSum[d][k] / heatNvCount[d][k]) * 10) / 10 : 0;
    });
  });

  const mealAvg = {}; const mealVegAvg = {}; const mealNvAvg = {};
  MEALS.forEach((m) => {
    mealAvg[m] = mealCount[m] > 0 ? Math.round((mealSum[m] / mealCount[m]) * 10) / 10 : 0;
    let vS=0, vC=0, nS=0, nC=0;
    DAYS.forEach(d => { vS += heatVegSum[d][MEAL_KEYS[m]]; vC += heatVegCount[d][MEAL_KEYS[m]]; nS += heatNvSum[d][MEAL_KEYS[m]]; nC += heatNvCount[d][MEAL_KEYS[m]]; });
  ...
  return {
    weekKey, weekLabel: feedbacks[0].week_label, weekRange: feedbacks[0].week_range,
    total: feedbacks.length,
    overallAvg: Math.round((MEALS.reduce((a,m) => a + mealAvg[m], 0)/4)*10)/10,
    mealAvg, mealVegAvg, mealNvAvg,
    bestMeal: sorted[0], worstMeal: sorted[3],
    heatmap, heatVeg, heatNv, ratingDist,
    comments: feedbacks.filter(f => f.liked || f.issues).map(f => ({ liked: f.liked, issue: f.issues, name: f.user_name, date: f.submitted_at }))
  };
}
...
passport.deserializeUser(async (id, done) => {
  try { const user = await User.findOne({ id }); done(null, user); } catch (e) { done(e, null); }
});
...
passport.use(new GoogleStrategy({
  ...
  try {
    ...
    if (!user) {
      user = await User.create({ id: genId(), name: profile.displayName, email, google_id: profile.id, picture: profile.photos[0].value, role });
    } else {
...
app.post("/api/auth/register", async (req, res) => {
  try {
    ...
    if (!isNistEmail(email)) return res.json({ success: false, message: `Only ${ALLOWED_DOMAIN} allowed.` });
    const hash = await bcrypt.hash(password, 12);
    const role = isAdminEmail(email) ? "admin" : "student";
    await User.create({ id: genId(), name, email: email.toLowerCase(), password_hash: hash, role });
...
app.post("/api/auth/login", async (req, res) => {
  ...
  req.login(user, () => res.json({ success: true, user: { name: user.name, email: user.email, role: user.role } }));
});
...
// ── FEEDBACK & HISTORY ───────────────────────────────────────
app.get("/api/week/current", (req, res) => res.json({ success: true, ...getCurrentWeekInfo() }));
  ...
  try {
    ...
    DAYS.forEach(day => {
      MEALS.forEach(meal => {
        ...
        if (val) meal_ratings.push({ day, meal, rating: Number(val), food_type: foodTypes[`${day}_${meal}`] || 'veg' });
      });
    });

    await Feedback.create({ id: genId(), user_email: email, user_name: name, week_key: wi.key, week_label: wi.label, week_range: wi.range, meal_ratings, liked, issues });
    res.json({ success: true });
  } catch (e) { res.json({ success: false, message: "Already submitted this week." }); }
});
...
app.get("/api/feedback/history", async (req, res) => {
  try {
    ...
    const fbs = await Feedback.find({ user_email: email.toLowerCase() }).sort({ submitted_at: -1 }).lean();
    
    const history = fbs.map(fb => {
      const h = {}; DAYS.forEach(d => { h[d] = {}; MEALS.forEach(m => { 
      ...
      const mealAvg = {};
      MEALS.forEach(m => {
        const vals = (fb.meal_ratings || []).filter(r => r.meal === m && r.rating > 0).map(r => r.rating);
        mealAvg[m] = vals.length > 0 ? Math.round((vals.reduce((a,b) => a+b, 0) / vals.length) * 10) / 10 : null;
      });
      const allVals = (fb.meal_ratings || []).filter(r => r.rating > 0).map(r => r.rating);
      const overallAvg = allVals.length > 0 ? Math.round((allVals.reduce((a,b) => a+b, 0) / allVals.length) * 10) / 10 : null;
      const total_rated = allVals.length;

      return { 
        week_key: fb.week_key, week_label: fb.week_label, week_range: fb.week_range,
        heatmap: h, mealAvg, overallAvg, total_rated,
        liked: fb.liked, issues: fb.issues,
...
app.get("/api/feedback/status", async (req, res) => {
  try {
    const email = req.query.email;
    if (!email) return res.status(400).json({ success: false, submitted: false });
    const wi = getCurrentWeekInfo();
    const fb = await Feedback.findOne({ user_email: email.toLowerCase(), week_key: wi.key }).lean();
  ...
  } catch (e) { res.status(500).json({ success: false, submitted: false }); }
});
...
const requireAdmin = async (req, res, next) => {
  ...
  // Fallback: check x-user-email header (used by frontend fetch calls)
...
app.get("/api/admin/complaints", requireAdmin, async (req, res) => {
  const data = await Feedback.find({ issues: { $ne: "" } }).sort({ submitted_at: -1 }).lean();
  res.json({ success: true, data: data.map(d => ({ name: d.user_name, email: d.user_email, week: d.week_label, text: d.issues, submittedAt: d.submitted_at })) });
});

app.get("/api/admin/submissions", requireAdmin, async (req, res) => {
  const { key } = getCurrentWeekInfo();
  const data = await Feedback.find({ week_key: key }).select("user_name user_email submitted_at").sort({ submitted_at: -1 }).lean();
  res.json({ success: true, data });
});

// ── RESTORED PDF EXPORT ROUTE (LARGE TEMPLATE) ───────────────
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    /* ── PRINT HEADER (logo on every page top-right) ── */
    @media print {
      ...
      /* Logo pinned top-right on every page */
      .page-logo-corner {
...
<body>
  ...
  <!-- Cinematic watermark logo (screen only, centred, low opacity, blur) -->
  <div class="watermark-wrap" aria-hidden="true">
    <img class="watermark-logo" src="${LOGO_URI}" alt="">
  </div>

  <!-- Small logo pinned top-right on every PRINT page -->
  <img class="page-logo-corner" src="${LOGO_URI}" alt="NIST Logo">
  ...
  <div class="page">
    ...
    <div class="meal-table-wrap">
      ...
      ${["breakfast","lunch","snacks","dinner"].sort((a,b) => an.mealAvg[b]-an.mealAvg[a]).map(m => {
        const sc = an.mealAvg[m] || 0;
        const barClr = sc >= 4 ? "#22c55e" : sc >= 3 ? "#eab308" : "#ef4444";
        const tag = sc >= 4 ? "Performing well" : sc >= 3 ? "Needs improvement" : "Critical";
    ...
    <div class="comments-wrap">
      ...
      ${an.comments.slice(0,10).map((c,i) => `
...
// ── SERVER START ─────────────────────────────────────────────
initDB().then(() => {
  app.listen(PORT, () => console.log(`🚀 ${UNIVERSITY_NAME} Messify Online at Port ${PORT}`));
});